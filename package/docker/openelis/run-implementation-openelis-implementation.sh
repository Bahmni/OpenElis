...
java $CHANGE_LOG_TABLE -jar $LIQUIBASE_JAR --classpath=$CLASSPATH --driver=$DRIVER --changeLogFile=$CHANGE_LOG_FILE --defaultSchemaName=clinlims --url=jdbc:postgresql://$OPENELIS_DB_SERVER:$OPENELIS_DB_PORT/$OPENELIS_DB_NAME --username=$OPENELIS_DB_USERNAME --password=$OPENELIS_DB_PASSWORD update
cd -
else
