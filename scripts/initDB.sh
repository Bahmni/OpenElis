...
if [ "$(psql -Upostgres -h $OPENELIS_DB_SERVER -lqt | cut -d \| -f 1 | grep -w $DATABASE_NAME | wc -l)" -eq 0 ]; then
    ...
    psql -U postgres -h $OPENELIS_DB_SERVER -f $SCRIPTS_DIR/database/setupDB.sql
...
psql -U postgres -h $OPENELIS_DB_SERVER -d clinlims -f $SCRIPTS_DIR/database/setupExtensions.sql