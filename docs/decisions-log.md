...
| **`bahmni-reports` has a Direct DB connection to OpenELIS PostgreSQL** (confirmed by archDiagWithRepoLinks.png). All lab Jasper/BIRT reports querying the `clinlims` schema will break with OEG2's different schema. This is a go-live blocker requiring a repo deep dive (Open Question 7). | arch diagram review (2026-02-19) |
| **`bahmni-metabase` has a Direct DB connection to OpenELIS PostgreSQL** (confirmed by archDiagWithRepoLinks.png). All lab analytics dashboards (TAT, workload, test volumes) will break with OEG2's different schema. Go-live blocker (Open Question 8). | arch diagram review (2026-02-19) |
...
| **`openmrs-distro-bahmni`** is the Docker Compose packaging repo. Needs changes to add OEG2 containers and remove old OpenELIS containers. Not a risk — straightforward work for Phase 4. | arch diagram review (2026-02-19) |
